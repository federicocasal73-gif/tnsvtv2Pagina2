...
final class Version20260901000000 extends AbstractMigration
{
    ...
    {
        // Portable across MySQL/MariaDB and SQLite (CI uses sqlite via
        // schema introspection; MySQL needs AFTER/FK syntax that SQLite
        // rejects and vice versa for AUTOINCREMENT/ON CONFLICT).
        ...
        if ($schema->hasTable('tasks')) {
            if ($isMysql) {
                ...
                $this->addSql(<<<'SQL'
                    ...
                    ADD COLUMN status VARCHAR(32) NOT NULL DEFAULT 'pending'
                SQL);
                $this->addSql("ALTER TABLE tasks ADD COLUMN priority VARCHAR(16) NOT NULL DEFAULT 'normal'");
                $this->addSql("ALTER TABLE tasks ADD COLUMN type VARCHAR(32) NOT NULL DEFAULT 'general'");
                $this->addSql('ALTER TABLE tasks ADD COLUMN assigned_to_id INT DEFAULT NULL');
                $this->addSql('ALTER TABLE tasks ADD COLUMN assigned_by_id INT DEFAULT NULL');
                $this->addSql('ALTER TABLE tasks ADD COLUMN course_id INT DEFAULT NULL');
                $this->addSql('ALTER TABLE tasks ADD COLUMN lesson_id INT DEFAULT NULL');
                $this->addSql('ALTER TABLE tasks ADD COLUMN due_date DATETIME DEFAULT NULL');
                $this->addSql('ALTER TABLE tasks ADD COLUMN estimated_minutes INT DEFAULT NULL');
                $this->addSql('ALTER TABLE tasks ADD COLUMN instructions TEXT DEFAULT NULL');
                $this->addSql('ALTER TABLE tasks ADD COLUMN links JSON DEFAULT NULL');
                $this->addSql('ALTER TABLE tasks ADD COLUMN attachments JSON DEFAULT NULL');
        ...
        if ($schema->hasTable('campus_assignments') && $schema->hasTable('campus_lessons') && $schema->hasTable('campus_courses') && $schema->hasTable('users')) {
            ...
            // MySQL uses INSERT IGNORE, SQLite/Postgres use ON CONFLICT.
            $insertInto = $isMysql
                ? 'INSERT IGNORE INTO tasks (title, description, status, priority, type, assigned_to_id, course_id, lesson_id, due_date, instructions, orden, active, created_at, updated_at)'
                : 'INSERT INTO tasks (title, description, status, priority, type, assigned_to_id, course_id, lesson_id, due_date, instructions, orden, active, created_at, updated_at)';
            $this->addSql($insertInto . "
                SELECT
                    ca.title,
                    ca.description,
                    CASE
                ...
                . ($isMysql ? '' : ' ON CONFLICT DO NOTHING')
            );
        }
    }
    ...
    {
        $isMysqlDown = str_contains($this->connection->getDatabasePlatform()::class, 'MySQL');
        if ($schema->hasTable('task_feedback')) {
        ...
        if ($schema->hasTable('tasks')) {
            if ($isMysqlDown) {
                ...
                // SQLite: no DROP FOREIGN KEY / multi-action ALTER; drop
                // columns one by one, ignore FKs (SQLite enforces on delete
                // only when declared inline, which these are not).
                foreach (['status', 'priority', 'type', 'assigned_to_id',
                    'assigned_by_id', 'course_id', 'lesson_id', 'due_date',
                    'estimated_minutes', 'instructions', 'links', 'attachments',
                    'completed_at'] as $col) {
                    $this->addSql("ALTER TABLE tasks DROP COLUMN {$col}");
                }
            }
        }
    }
